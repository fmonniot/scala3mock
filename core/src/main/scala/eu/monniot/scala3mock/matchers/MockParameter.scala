...
class MockParameter[T] protected (
    val value: T | MatcherBase,
    dummy: Boolean = false
):
...
object MockParameter extends MockParameterImplicitL1:
  given matchAny[T]: Conversion[MatcherBase, MockParameter[T]] = t =>
    new MockParameter(t)
...
private class MockParameterImplicitL1:
  given specificValue[T]: Conversion[T, MockParameter[T]] = t =>
    new MockParameter(t)
