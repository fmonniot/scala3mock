...
trait Mocks:
  ...
  inline def when[R: Default](inline f: () => R): MockFunction0[R] = (${WhenImpl('f)})
  inline def when[T1, R: Default](inline f: T1 => R): MockFunction1[T1, R] = (${WhenImpl('f)})
